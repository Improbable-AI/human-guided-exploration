...
class Environment(gym.Env):
  ...
  def __init__(self,
    ...
    self.random_box_position = random_box_position
    print("Display here")
    ...
    if disp:
      print("Displaying trajectory on GUI")
      disp_option = p.GUI
      if shared_memory:
  ...
  def reset(self, poses={}):
    ...
    # Reset task.
    self.task.reset(self, poses, random_box_position=self.random_box_position, random_goal=self.random_goal)
  ...
  def movej(self, targj, speed=0.01, timeout=0.1): #TODO: here there was 5 before
    ...
    while i < self.wait_time:
      ...
      p.setJointMotorControlArray(
          ...
          positionGains=gains)
      p.stepSimulation()
    print(f'Warning: movej exceeded {time.time() - t0} second timeout. Skipping.')
    return True
  ...
  def _get_obs(self):
    ...
    #  obs['depth'] += (depth,)
    state = []
    for ids in self.obj_ids['rigid']:
        pos, orientation = p.getBasePositionAndOrientation(ids)
        state.append(pos)
        state.append(orientation)

    state = np.concatenate(state)

    goal = []
    # for ids in self.obj_ids['fixed']:
    #     pos, orientation = p.getBasePositionAndOrientation(ids)
    #     goal.append(pos)
    ...
    obs['state'] = state
    obs['goal'] = goal
    obs['suction_state'] = self.task.primitive.s_bit
    obs['object_grabbed'] = self.ee.check_grasp()
...
class ContinuousEnvironment(Environment):
  ...
  def step(self, action=None, max_wait=20):
    if action is not None:
      timeout = self.task.primitive(self.movej, self.movep, self.ee, action)
      # Exit early if action times out. We still return an observation
      # so that we don't break the Gym API contract.
    ...
    # Step simulator asynchronously until objects settle.
    i = 0
    while not self.is_static and i < max_wait:
      i+=1
      p.stepSimulation()
    # Get task rewards.
    reward, info = self.task.reward() if action is not None else (0, {})
