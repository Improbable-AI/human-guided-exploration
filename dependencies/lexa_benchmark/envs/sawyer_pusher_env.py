...
class SawyerPushGoalEnv(BenchEnv):
    def __init__(self, action_repeat=1, use_goal_idx=False, log_per_goal=False, fixed_start=True, fixed_goal=False, width=64, height=64, image_kwargs=None):
        super().__init__(
            action_repeat, width=width,
        )
        ...
        self._env = SawyerPushAndReachXYEnv(**push_configs[config_key])
        self.num_envs = 1
        self.width = width
        self.height = height
    ...
    def compute_shaped_distance(self, achieved_state, goal):
        ...
        achieved_state_puck = achieved_state[2:4]
    ...
    def get_obs_internal(self, state):
        return state[:4]
    ...
    # TODO: be careful about at which state is the end effector position is expected to be
    def render_state(self, state):
        ...
        self._env.set_to_goal({'state_desired_goal': state})
        state_image = self._env.render('rgb_array')
    ...
    def _get_obs(self, state):
        ...
        distance = self.compute_shaped_distance(np.array(obs_state), np.array(goal))
        obs['total_distance'] = distance
        obs['total_success'] = int(distance < 0.05)
        return obs
    ...
    def compute_reward(self, goal=None):
        ...
        achieved_state = self._env._get_obs()['observation'] #self._env.sim.data.qpos.copy()
    ...
    def compute_success(self, state, goal):
        return int(self.compute_shaped_distance(state, goal) < 0.05)